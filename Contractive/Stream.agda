...
ofe = record
  ...
  ; Eq                 = \n xs ys -> take (suc n) xs ≡ take (suc n) ys
  ; isEquivalence      = trans'
  }
  where
  trans' = \_ -> record
...
private
  ...
  isLimitU s coh {zero}  _ = begin
    take 1 (s 0)     ≡⟨ ≡-cong (take 1) η ⟩
    head (s 0) ∷ []  ∎
    where open ≡-Reasoning
  isLimitU s coh {suc n} _ = begin
    take (2 + n) (s (suc n))                                 ≡⟨ ≡-cong (take (2 + n)) η ⟩
    head (s (suc n)) ∷ take (1 + n) (tail (s (suc n)))       ≡⟨ ≡-cong₂ _∷_ lem₁ (isLimitU (tail ∘ s ∘ suc)
                                                                                           (step s coh) {n} _) ⟩
    head (s 0)       ∷ take (1 + n) (limU (tail ∘ s ∘ suc))  ∎
    where
    open ≡-Reasoning

    lem₂ : forall {n} (xs ys : Stream A) ->
          take (suc n) xs ≡ take (suc n) ys ->
          head xs ≡ head ys
    lem₂ (x ≺ xs) (y ≺ ys) x≺xs≡y≺ys = ≡-cong Vec.head x≺xs≡y≺ys

    lem₁ : head (s (suc n)) ≡ head (s 0)
    lem₁ = lem₂ _ _ $ ≡-sym $ coh {0} {suc n} _ _ (s≤′s z≤′n)

  lim↓ : A -> forall n -> (forall n' -> n' <′ n -> Stream A) -> Stream A
  lim↓ x zero    s = repeat x
  lim↓ x (suc n) s = s n ≤′-refl

  isLimit↓ : forall x n (s : Family (↓ n)) ->
