...
fibF = record
  ...
  where
  F = \xs -> 0 ≺ 1 ≺ zipWith _+_ xs (tail xs)

  lemma₁ : forall _∙_ (xs ys : Stream ℕ) n ->
  ...
  isCon : forall (xs ys : Stream ℕ) n ->
          (forall {m} -> m <′ n -> Eq m xs ys) ->
          Eq n (F xs) (F ys)
...
hammingF = record
  ...
  F = \xs -> 0 ≺ merge cmp (map (_*_ 2) xs) (map (_*_ 3) xs)

  postulate
    lemma : forall n -> cmp (2 * suc n) (3 * suc n) ≡ lt

  isCon : forall (xs ys : Stream ℕ) n ->
          (forall {m} -> m <′ n -> Eq m xs ys) ->
          Eq n (F xs) (F ys)
  isCon _            _               zero    _   = ≡-refl
  isCon (x ≺ xs)     ( y ≺ ys)       (suc n) hyp
    with ≡-cong Vec.head (hyp (s≤′s z≤′n))
  ...
  isCon (suc x ≺ xs) (.(suc x) ≺ ys) (suc n) hyp | ≡-refl | .lt | ≡-refl =
    ≡-cong (\zs -> 0 ∷ 2 * suc x ∷ zs) (begin
      take n (merge cmp (map (_*_ 2) xs)
                        (3 * suc x ≺ map (_*_ 3) xs))  ≡⟨ iCantBeBothered ⟩
      take n (merge cmp (map (_*_ 2) ys)
                        (3 * suc x ≺ map (_*_ 3) ys))  ∎)
    where postulate iCantBeBothered : _
