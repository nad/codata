...
mutual
  ...
  data _≅_ {A} : (xs ys : Stream A) -> Set1 where
    ...
    map-cong     : forall {B}
                   ...
                   map f xs ≅ map f ys
    zipWith-cong : forall {B C} (_∙_ : B -> C -> A)
                   {xs xs′} (xs≈xs′ : xs ≅ xs′)
                   {ys ys′} (ys≈ys′ : ys ≅ ys′) ->
...
≅⇒≃ (map-cong f {xs} {ys} xs≈ys) with xs | ys | ≅⇒≃ xs≈ys
≅⇒≃ (map-cong f xs≈ys)           | x ≺ xs′ | y ≺ ys′ | x≡y ≺ xs≈ys′ =
                                   ≡-cong f x≡y ≺ map-cong f xs≈ys′
≅⇒≃ (zipWith-cong ∙ {xs} {xs′} xs≈xs′ {ys} {ys′} ys≈ys′)
  with xs | xs′ | ≅⇒≃ xs≈xs′ | ys | ys′ | ≅⇒≃ ys≈ys′
... | _ ≺ _ | _ ≺ _ | x≡y ≺ xs≈ys | _ ≺ _ | _ ≺ _ | x≡y′ ≺ xs≈ys′ =
  ≡-cong₂ ∙ x≡y x≡y′ ≺ zipWith-cong ∙ xs≈ys xs≈ys′
